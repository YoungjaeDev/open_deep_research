...
SUMMARIZATION_PROMPT_KO = """웹 검색으로 검색된 웹페이지의 원시 콘텐츠를 요약하는 작업을 수행합니다. 원본 웹페이지에서 가장 중요한 정보를 보존하는 간결한 요약을 만드는 것이 목표입니다. 이 요약은 다운스트림 연구 에이전트가 사용할 것이므로, 필수 정보를 잃지 않으면서 핵심 세부사항을 유지하는 것이 중요합니다.
...
2. 콘텐츠의 메시지에 핵심이 되는 주요 사실, 통계, 데이터 포인트를 유지하세요.
3. 신뢰할 만한 출처나 전문가의 중요한 인용문을 보존하세요.
4. 콘텐츠가 시간에 민감하거나 역사적인 경우 사건의 시간순 순서를 유지하세요.
5. 목록이나 단계별 지침이 있다면 보존하세요.
6. 콘텐츠 이해에 중요한 관련 날짜, 이름, 위치를 포함하세요.
7. 긴 설명을 요약하되 핵심 메시지는 그대로 유지하세요.

다양한 유형의 콘텐츠 처리 시:

- 뉴스 기사의 경우: 누가, 무엇을, 언제, 어디서, 왜, 어떻게에 집중하세요.
- 과학적 콘텐츠의 경우: 방법론, 결과, 결론을 보존하세요.
- 의견 글의 경우: 주요 논거와 뒷받침하는 요점들을 유지하세요.
- 제품 페이지의 경우: 주요 기능, 사양, 고유한 판매 포인트를 보존하세요.

요약은 원본 콘텐츠보다 상당히 짧아야 하지만 정보 소스로서 독립적으로 기능할 수 있을 만큼 포괄적이어야 합니다. 콘텐츠가 이미 간결한 경우가 아니라면 원본 길이의 약 25-30%를 목표로 하세요.

다음 형식으로 요약을 제시하세요:

```
{{
   "summary": "적절한 단락이나 불릿 포인트로 구성된 간결한 요약",
   "key_excerpts": [
     "첫 번째 중요한 인용문이나 발췌문",
     "두 번째 중요한 인용문이나 발췌문", 
     "세 번째 중요한 인용문이나 발췌문",
     ...필요에 따라 더 많은 발췌문 추가, 최대 5개까지
   ]
}}
```
...
```json
{{
   "summary": "2023년 7월 15일, NASA는 케네디 우주센터에서 아르테미스 II 미션을 성공적으로 발사했습니다. 이는 1972년 아폴로 17호 이후 첫 번째 유인 달 탐사 미션입니다. 제인 스미스 사령관이 이끄는 4명의 승무원은 10일간 달 궤도를 돌고 지구로 돌아올 예정입니다. 이 미션은 2030년까지 달에 영구적인 인간 거주지를 설립하려는 NASA 계획의 중요한 단계입니다.",
   "key_excerpts": [
     "아르테미스 II는 우주 탐사의 새로운 시대를 나타냅니다"라고 NASA 관리자 존 도가 말했습니다.
     "이 미션은 향후 달에서의 장기 체류를 위한 중요한 시스템들을 테스트할 것입니다"라고 수석 엔지니어 사라 존슨이 설명했습니다.
     "우리는 단지 달로 돌아가는 것이 아니라, 달로 앞으로 나아가는 것입니다"라고 제인 스미스 사령관이 발사 전 기자회견에서 말했습니다.
   ]
}}
```
...
```json
{{
   "summary": "Nature Climate Change에 발표된 새로운 연구에 따르면 전 세계 해수면이 이전 예상보다 빠르게 상승하고 있습니다. 연구자들은 1993년부터 2022년까지의 위성 데이터를 분석한 결과, 지난 30년 동안 해수면 상승률이 연간 0.08mm/년²씩 가속화되었음을 발견했습니다. 이러한 가속화는 주로 그린란드와 남극의 빙상 융해에 기인합니다. 연구는 현재 추세가 계속될 경우 2100년까지 전 세계 해수면이 최대 2미터까지 상승할 수 있어 전 세계 연안 지역사회에 심각한 위험을 초래할 것이라고 예측합니다.",
   "key_excerpts": [
      "우리의 발견은 해수면 상승의 명확한 가속화를 보여주며, 이는 연안 계획 및 적응 전략에 중요한 의미를 갖습니다"라고 주저자 에밀리 브라운 박사가 말했습니다.
      "그린란드와 남극의 빙상 융해율이 1990년대 이후 3배 증가했습니다"라고 연구는 보고합니다.
      "온실가스 배출의 즉각적이고 실질적인 감소 없이는 금세기 말까지 잠재적으로 재앙적인 해수면 상승을 보게 될 것입니다"라고 공동저자 마이클 그린 교수가 경고했습니다.
   ]
}}
```

기억하세요. 목표는 원본 웹페이지의 가장 중요한 정보를 보존하면서 다운스트림 연구 에이전트가 쉽게 이해하고 활용할 수 있는 요약을 만드는 것입니다."""
